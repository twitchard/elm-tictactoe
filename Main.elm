...
type Msg = Noop | Click Int Int | ReceiveUpdate Board | RoomName String | SubmitRoomName | Debug String
...
setInBoard marker x y board =
  case Array.get y board of
    Nothing -> board
    Just row -> Array.set y (Array.set x (Just marker) row) board

update : Msg -> Model -> (Model, Cmd Msg)
update msg model = case msg of
  Noop -> (model, Cmd.none)
  Click x y -> 
    ({ model | board = setInBoard X x y model.board
    }, WebSocket.send wsURL "yo")
  RoomName s -> ({model | roomName = s}, Cmd.none)
  SubmitRoomName -> (model, WebSocket.send wsURL "{}")
  ReceiveUpdate board -> ({model | board = board}, Cmd.none)
  Debug string -> ({model | debug = string}, Cmd.none)
...
renderCell yindex xindex cell = 
  let letter = case cell of 
                 Just X -> "x"
                 Just O -> "o"
                 Nothing -> ""
      leftCornerX = toString <| xindex * 100
      leftCornerY = toString <| yindex * 100
      centerX = toString <| xindex * 100 + 50
      centerY = toString <| yindex * 100 + 50
  in
    Svg.g []
      [ Svg.rect
        [ x leftCornerX
...
processMessage string = Debug string
